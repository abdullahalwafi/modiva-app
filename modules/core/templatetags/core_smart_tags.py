...
def get_user_menu2(user, parent_id):

    user_menu = []
    second_menu = []

    if user.is_superuser:
        second_menu = Menu.objects.filter(aktif=True,aplikasi__aktif=True,parent=parent_id).order_by('urutan','name').prefetch_related()
        user_menu = Menu.objects.filter(aktif=True,aplikasi__aktif=True).exclude(parent=parent_id).order_by('urutan','name').prefetch_related()
        ...
        for gm in group_menu:
            for m_ in gm.menu.all().exclude(Q(aplikasi__aktif=False)|Q(aktif=False)).order_by('urutan','name').prefetch_related():
                 if m_.parent == None:
                       if m_ in second_menu:
                          continue
                       else:
                          second_menu.append(m_)
                 else:
                       #print(f' OKEEEEEEE {m_}')
                       if m_ in user_menu:
                          continue
                       else:
                          user_menu.append(m_)
     
    menu_all = (second_menu,user_menu)  
    #print(f' JJJJ {menu_all}')
    return menu_all

def get_active_menu(path,user):
    menu_active = None
    id_embed = None
    topmenu,gmenu = get_user_menu(user)
    menu_id = [ t.id for t in topmenu ] + [ g.id for g in gmenu ] 
    if path != '':
        id_embed = id_embed_from_url(path)
        if id_embed:
                  menu_active = Menu.objects.filter(id=id_embed,aktif=True,aplikasi__aktif=True,id__in=[m for m in menu_id]).first()
        else:
             menu_active = Menu.objects.filter(aktif=True,aplikasi__aktif=True,id__in=[m for m in menu_id]).filter(Q(url__exact=path)|Q(url__exact=get_relative_path(path))).first()
    if not menu_active:
        # coba resolve path, siapa tahu ada route dgn parameter (kwargs)
        try:
            resolved_path = resolve(path)
            pattern = ""
            for k in resolved_path.kwargs.keys():
                pattern += f"<.*:{k}>/"
            rel_path = get_relative_path(resolved_path.route)
            if not rel_path:
                rel_path = get_relative_path("/" + resolved_path.route)
            if rel_path:
                match = re.search(f"(.*/){pattern}", rel_path)
                path = match.group(1)
                if path:
                    menu_active = Menu.objects.filter(
                        aktif=True, aplikasi__aktif=True,
                        id__in=[m for m in menu_id]).filter(url__exact=path).first()
        except Exception:
            ...
    #print('### Menu User: ',gmenu)
...
def show_ng_menu(path='',user=None):
    ...
    #print('$$$$$$$ PATH: ',path)
    menu_active = get_active_menu(path,user)
    for tm in top_menu:
    ...
    # html = html + "</div></div></div>\n"  # coba comment karena bikin layout break (content di tengah page, tidak align ke top)
    context['menu'] = html
    return context
...
def show_topmenu2(path='',user=None):
    top_menu = []
    ...
    for tm in top_menu:
        ...
        if not menu_have_childs(tm):
            html = html + """<div data-kt-menu-trigger="{default: 'click', lg: 'hover'}" data-kt-menu-placement="bottom-start" class="menu-item menu-lg-down-accordion menu-sub-lg-down-indention me-0 me-lg-2"><span class="menu-link"><span class="menu-title"><a class="nav-link %s" data-bs-toggle="" href="%s" %s>%s</a></span><span class="menu-arrow d-lg-none"></span></span>
</div>"""%(active,set_absolute_path(tm.url),target,tm.name)  
        else:
            html = html + """<div data-kt-menu-trigger="{default: 'click', lg: 'hover'}" data-kt-menu-placement="bottom-start" class="menu-item menu-lg-down-accordion menu-sub-lg-down-indention me-0 me-lg-2"><span class="menu-link"><span class="menu-title"><a class="nav-link %s" data-bs-toggle="tab" href="#kt_header_navs_tab_%s">%s</a></span><span class="menu-arrow d-lg-none"></span></span>"""%(active,tm.id,tm.name)
            second_menu = []
            second_menu,gmenu = get_user_menu2(user,tm.id)
            #print(f' OKE {second_menu} - {user} - {tm.id}')
            second_menu_berurut = Menu.objects.filter(id__in=[ menu_.id for menu_ in second_menu ],aktif=True).order_by('urutan','name')
            for menu_ in second_menu :

                #print(f' OKE2 {menu_.id} ')
                second_menu = second_menu_berurut
            if second_menu:
                html = html + """<div class="menu-sub menu-sub-lg-down-accordion menu-sub-lg-dropdown px-lg-2 py-lg-4 w-lg-250px">
        <!--begin:Menu item-->"""
                for tm2 in second_menu:
                    #print(f' CCC {tm2}')
                    menus = Menu.objects.filter(parent=tm2,aktif=True)
                    for tm3 in menus:
                        #print(f' MENUS {tm3.url}')
                        if not menu_have_childs(tm2):
                            html = html + """<!--begin:Menu link--><div data-kt-menu-trigger="{default:'click', lg: 'hover'}" data-kt-menu-placement="right-start" class="menu-item menu-lg-down-accordion">
                <span class="menu-item"><a class="menu-link" href="%s">
                    <span class="menu-icon">
                        <i class="ki-outline ki-basket fs-2"></i>
                    </span>
                    <span class="menu-title">%s</span></a>        
                </span></div>"""%(set_absolute_path(tm2.url),tm2.name)
                        else:                         
                           html = html + """<!--begin:Menu link--><div data-kt-menu-trigger="{default:'click', lg: 'hover'}" data-kt-menu-placement="right-start" class="menu-item menu-lg-down-accordion">
                <span class="menu-item"><a class="menu-link" href="%s">
                    <span class="menu-icon">
                    ...
                    <span class="menu-title">%s</span></a>        
                </span></div>"""%(set_absolute_path(tm3.url),tm3.name)
    ...
    return context
show_topmenu_template = get_template('core/topmenu2.html')  
register.inclusion_tag(show_topmenu_template)(show_topmenu2)
...
def format_rupiah(value):
    return "Rp{:,.0f}".format(value) if value else None


def show_sidebar(path='', user=None):
    sidebar_menu = []
    sidebar_menu, gmenu = get_user_menu(user)
    menu_active = get_active_menu(path, user)

    sidebar_menu_berurut = Menu.objects.filter(
        id__in=[menu_.id for menu_ in sidebar_menu],
        aktif=True
    ).order_by('urutan', 'name')
    sidebar_menu = sidebar_menu_berurut

    context = {}
    html = ''

    for sm in sidebar_menu:
        url, target, active, here, text_primary, text_style = get_active_attrs(sm, menu_active)

        if not menu_have_childs(sm):
            html += f"""
            <div class="menu-item">
                <a href="{set_absolute_path(sm.url)}" class="menu-link {active}" {target}>
                    <span class="menu-icon">
                        <i class="ki-outline ki-basket fs-2"></i>
                    </span>
            ...
            second_menu, gmenu = get_user_menu2(user, sm.id)
            second_menu_berurut = Menu.objects.filter(
                id__in=[menu_.id for menu_ in second_menu],
                aktif=True
            ).order_by('urutan', 'name')
            second_menu = second_menu_berurut

            for sm2 in second_menu:
                if not menu_have_childs(sm2):
                    html += f"""
                    <div class="menu-item">
                        <a href="{set_absolute_path(sm2.url)}" class="menu-link">
                            <span class="menu-bullet">
                                <span class="bullet bullet-dot"></span>
                            </span>
                            <span class="menu-title">{sm2.name}</span>
                        </a>
                    </div>
                    """
                else:
                    # 3rd level menu if needed
                    third_menu = Menu.objects.filter(parent=sm2, aktif=True)
                    html += f"""
                    <div class="menu-item menu-accordion">
                        <span class="menu-link">
                            <span class="menu-bullet">
                        ...
                        <div class="menu-sub menu-sub-accordion">
                    """
                    for sm3 in third_menu:
                        html += f"""
                        <div class="menu-item">
                            <a href="{set_absolute_path(sm3.url)}" class="menu-link">
                                <span class="menu-bullet">
                                    <span class="bullet bullet-dot"></span>
                                </span>
                                <span class="menu-title">{sm3.name}</span>
                            </a>
                        </div>
                        """
                    html += "</div></div>"
...
# Register the tag with your sidebar template
show_sidebar_template = get_template('core/appmenu2.html')
register.inclusion_tag(show_sidebar_template)(show_sidebar)
